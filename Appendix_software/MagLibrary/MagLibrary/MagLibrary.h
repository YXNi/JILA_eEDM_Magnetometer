...
class MagLibrary  //public Adafruit_Sensor
{
  public:
    MagLibrary();
    void begin(magvec*);
    void setMagGain(hmc5883MagGain gain);
    void getEvent(magvec*);
	  void setStrap(bool);
	  void cancelStrap(void);
	  void dGauss(int strapCounts);
    void getStrap(magvec*);
    void checkState(void);
    bool state;

  private:
    hmc5883MagGain   _magGain;
    magvec		     _magData;     // Last read magnetometer data will be available here

	  unsigned t1; //time counters
	  unsigned t2;
	  int lp;//the registered loops
